\documentclass[a4paper,openany,12pt]{book}
\input{contenido/Incluir.tex}

% Colocar los argumentos definidos
\faculty{INGENIERÍA ELÉCTRICA, ELECTRÓNICA, INFORMÁTICA Y MECÁNICA}
\school{INGENIERÍA INFORMÁTICA Y DE SISTEMAS}
\title{Plataforma Web para la Optimización de la Gestión del TUPA de la UNSAAC}
\memberOne{Cabeza Huillca Flavio Antony}
\codeOne{211265}
\memberTwo{Mamani Flores Natan}
\codeTwo{230970}
\memberThree{Polo Chura Marco Rosauro}
\codeThree{141632}
\memberFour{Ramos Alata Hernan Washington}
\codeFour{221989}
\memberFive{Vitorino Marin Efrain}
\codeFive{160337}

\docente{Ing. Quispe Sota Julio Vladimir}

%Iniciar el documento
\begin{document}
% Iniciar numeración en romanos
\pagenumbering{roman}
\maketitle % Caratula
\doublespacing
\input{contenido/presentacion}

\newpage
\addcontentsline{toc}{chapter}{ÍNDICE}
\tableofcontents

\newpage
\addcontentsline{toc}{chapter}{ÍNDICE DE TABLAS}
\listoftables

\newpage
\addcontentsline{toc}{chapter}{ÍNDICE DE FIGURAS}
\listoffigures

\setlength{\tabcolsep}{10pt}
\clearpage
\pagenumbering{arabic} % Numeracion normal

\input{contenido/cap_1_ambito}
\input{contenido/cap_2_problematica}
\input{contenido/cap_3_objetivos}
\input{contenido/cap_4_metodologia}
\input{contenido/cap_5_infraestructura}
\input{contenido/cap_6_evaluacion}
\input{contenido/cap_7_resultados}
\input{contenido/cap_8_participantes}
\input{contenido/cap_9_aspectos_admin}
\input{contenido/cap_10_cronograma}
\input{contenido/cap_11_presupuesto}
\input{contenido/cap_12_prototipado}

\end{document}
